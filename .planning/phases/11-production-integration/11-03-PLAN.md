...
4. If there's also a JobStatusResponse that includes nmr_results, ensure the NMRResultsResponse is properly constructed there too.

Note: The existing code may build NMRResultsResponse differently. Adapt the pattern to fit - the key is adding the three new fields with proper values.
  </action>
  <verify>
