...
must_haves:
  truths:
    - "User can deploy to DigitalOcean/Linode VPS following the guide"
    - "User knows how to configure HTTPS with a custom domain"
    - "User can diagnose common Docker deployment issues"
    - "User can backup and restore job data across server migrations"
  artifacts:
  ...
  key_links:
    ...
    - from: "docs/deployment.md"
      ...
      via: "service architecture"
      pattern: "docker.compose"
---

<objective>
Create comprehensive deployment guide covering cloud VPS setup, HTTPS configuration, troubleshooting, and backup/restore procedures.

Purpose: Users need clear guidance for production deployments beyond localhost. This guide covers the full journey from provisioning a VPS to running with auto-HTTPS and maintaining data.

Output: docs/deployment.md with cloud VPS guide, troubleshooting section, and backup/restore instructions
</objective>
...
<task type="auto">
  <name>Task 1: Create docs/deployment.md with VPS Guide</name>
  <files>docs/deployment.md</files>
  <action>
Create docs/deployment.md with the following comprehensive structure:
...
This creates a comprehensive deployment guide covering DOCS-02 (cloud VPS), DOCS-03 (troubleshooting), and DOCS-04 (backup/restore).

Key sections based on v2.4 research pitfalls:
- X11 libraries for RDKit drawing (Phase 36)
- SIGINT/graceful shutdown notes (Phase 37)
- MPI/shm_size troubleshooting (Phase 37)
- Worker amd64-only note implied in architecture
  </action>
  <verify>
- docs/deployment.md exists with 200+ lines
- Contains "Cloud VPS Deployment" section
- Contains "Troubleshooting" section
- Contains "Backup and Restore" section
- References .env configuration
  </verify>
  <done>
Comprehensive deployment guide exists covering cloud VPS setup (DigitalOcean, Linode, etc.), HTTPS configuration, troubleshooting common issues from v2.4 research, and backup/restore procedures for job data.
  </done>
</task>

<task type="auto">
  <name>Task 2: Update docs/README.md to include deployment guide</name>
  <files>docs/README.md</files>
  <action>
Update docs/README.md to include the new deployment guide in the documentation index:

Add "Deployment Guide" link between Installation Guide and Usage Guide (or in appropriate location based on current structure).

If docs/README.md doesn't exist, this task is complete (the main README.md links to docs/deployment.md directly).

Check if docs/README.md exists first. If it does, add:

```markdown
- **[Deployment Guide](deployment.md)** - Docker deployment, cloud VPS setup, and troubleshooting
```
  </action>
  <verify>
- If docs/README.md exists: contains link to deployment.md
- If docs/README.md doesn't exist: task complete (main README handles links)
  </verify>
  <done>
Documentation index updated to include deployment guide, or confirmed that main README.md provides the documentation navigation.
  </done>
</task>
...
<verification>
- [ ] docs/deployment.md exists with comprehensive content
- [ ] Cloud VPS section covers DigitalOcean/Linode/Hetzner
- [ ] HTTPS configuration documented with Caddy
- [ ] Troubleshooting section covers common Docker issues
- [ ] Backup and restore procedures documented
- [ ] All commands are accurate and copy-pasteable
</verification>

<success_criteria>
- User can follow guide to deploy on a cloud VPS
- User can configure HTTPS with custom domain
- User can diagnose common deployment issues
- User can backup and restore job data
- Documentation references actual files (.env.example, docker-compose.yml)
</success_criteria>
