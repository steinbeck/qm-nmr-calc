...
must_haves:
  ...
  artifacts:
    - path: "src/qm_nmr_calc/nmredata.py"
      ...
      exports: ["generate_nmredata_sdf", "NMREDATA_VERSION", "NMREDATA_SEP", "format_sdf_tag"]
      min_lines: 150
...
2. Implementation steps (reuse pattern from jobs.py lines 935-965):
   ...
   h. Generate complete SDF with tags:
      - IMPORTANT: `MolToMolBlock()` only returns structure, NOT properties
      - Get MOL block: `mol_block = Chem.MolToMolBlock(mol)`
      - Manually append each tag in SDF format:
        ```
        >  <NMREDATA_VERSION>
        1.1
        ...
        $$$$
        ```
      - Helper function `format_sdf_tag(name: str, value: str) -> str` returns formatted tag block
      - Alternatively: use `io.StringIO` + `Chem.SDWriter` but manual formatting is simpler for single molecule
   i. Return complete SDF string (MOL block + all tags + $$$$)
...
# Verify complete SDF structure
assert 'M  END' in sdf, 'MOL block missing M  END'
assert '>  <NMREDATA_VERSION>' in sdf, 'Missing tag format'
assert '$$$$' in sdf, 'Missing SDF terminator'
print('Complete SDF structure valid!')
"
```
  </verify>
  <done>
