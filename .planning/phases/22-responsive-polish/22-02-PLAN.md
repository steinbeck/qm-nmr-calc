...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
**New in Phase 22 (this phase):**
- Touch-safe hover states (glass-card.css) - only on pointer: fine devices
- GPU-accelerated shadow animations (glass-card.css) - opacity-based pseudo-element
- Touch press feedback on mobile (glass-card.css) - scale(0.98) on :active
- Reduced motion support across all components (glass-card.css, status-page.css)
- Gap refinement: 12px (mobile) -> 16px (tablet) -> 24px (desktop)

**Already complete from Phase 18 (verify still works):**
- LAYOUT-01: Responsive bento grid (1/3/6 columns)
- LAYOUT-02: Card resizing at breakpoints
- LAYOUT-04: Collapsible sections work at all sizes
  </what-built>
  <how-to-verify>
1. Start development server: `cd /home/chris/develop/qm-nmr-calc && python -m flask --app src/qm_nmr_calc/api/app run --debug`

**NEW FEATURES TO TEST (Phase 22 scope):**

2. Test touch-safe hover behavior (desktop):
...
3. Test touch behavior (mobile emulation):
   ...
   - Verify brief press feedback on :active (slight scale down)
...
5. Test gap scaling (LAYOUT-03):
   - Resize browser width and check gap between cards:
   - < 768px (mobile): Gap should be 12px (0.75rem)
   - 768-1024px (tablet): Gap should be 16px (1rem)
   - > 1024px (desktop): Gap should be 24px (1.5rem)

**EXISTING FEATURES TO VERIFY (from Phase 18):**

6. Verify responsive grid still works (LAYOUT-01, LAYOUT-02):
   - Resize browser: 320px -> 768px -> 1024px+
   - Grid columns: 1 (mobile) -> 3 (tablet) -> 6 (desktop)
   - Cards resize appropriately at each breakpoint

7. Verify collapsible sections (LAYOUT-04):
   - Test expand/collapse on sections at mobile, tablet, desktop
   - Should work smoothly at all sizes
  </how-to-verify>
...
- [ ] Gap scales correctly: 12px -> 16px -> 24px at breakpoints
- [ ] Existing responsive grid still works (Phase 18 LAYOUT-01, 02, 04)
