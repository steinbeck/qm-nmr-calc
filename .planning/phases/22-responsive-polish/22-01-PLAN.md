...
must_haves:
  truths:
    ...
    - "Desktop breakpoint (1025px+) uses full gap value (24px)"
  artifacts:
    - path: "src/qm_nmr_calc/api/static/css/tokens.css"
      provides: "Tablet and desktop breakpoint token overrides"
...
Output: Updated tokens.css with tablet and desktop breakpoints, updated glass-card.css with touch-safe hover, GPU shadow animation, touch press states, and reduced motion support.
</objective>
...
<task type="auto">
  <name>Task 1: Add tablet and desktop breakpoint tokens and reduced motion token</name>
  <files>src/qm_nmr_calc/api/static/css/tokens.css</files>
  <action>
Add tablet and desktop breakpoint overrides between the existing mobile and desktop defaults in tokens.css:
...
2. Add desktop breakpoint override for 24px gap (REQUIRED for LAYOUT-03):
   ```css
   /* Desktop overrides */
...
3. Add reduced motion token near the transitions section (around line 94):
   ```css
...
The desktop override at 1025px ensures gap is 24px on large screens (not falling back to default 1rem).
  </action>
  <verify>
Verify tokens.css has:
- `@media (min-width: 768px) and (max-width: 1024px)` block with `--bento-gap: 1rem`
- `@media (min-width: 1025px)` block with `--bento-gap: 1.5rem`
- `--transition-reduced` variable defined in :root
  </verify>
  <done>Gap scales correctly: 12px (mobile) -> 16px (tablet) -> 24px (desktop). Reduced motion token available for other components.</done>
</task>
...
- [ ] tokens.css has desktop breakpoint with --bento-gap: 1.5rem (24px)
