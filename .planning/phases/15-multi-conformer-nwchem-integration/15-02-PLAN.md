...
2. `test_conformer_dft_optimization_all_succeed()` - Create ensemble with 3 conformers. Mock run_calculation to return a dict with this exact structure:
   ```python
   {
       "optimization_output": Path("<tmp>/optimize.out"),  # Path to a temp file whose .read_text() returns a string containing "Total DFT energy =     -40.5186"
       "geometry_file": Path("<tmp>/optimized.xyz"),        # Path to optimized geometry file
       "shielding_data": {...},                             # Present but unused in DFT step
       "shielding_output": Path("<tmp>/shielding.out"),     # Present but unused in DFT step
   }
   ```
   Create the temp file for optimization_output so that `.read_text()` returns text containing the energy line that extract_dft_energy can parse. After call:
...
Add `scratch_dir_override: Path | None = None` as the LAST parameter (after `on_optimization_complete`) to maintain backward compatibility with existing positional callers.

Also change `smiles: str` to `smiles: str | None` to support geometry_file input without SMILES. This is the path used for conformer optimization -- we have XYZ geometry from RDKit, want DFT optimization.
...
def run_calculation(
    ...
    scratch_dir_override: Path | None = None,  # NEW - last param for backward compat
) -> dict:
```

**Checklist for smiles=None handling:**
1. Change signature from `smiles: str` to `smiles: str | None`
2. Update validation: at the start of the function body, add check:
   ```python
   if smiles is None and geometry_file is None:
       raise ValueError("Either smiles or geometry_file must be provided")
   ```
3. Add if/else branch in the `not skip_optimization` path:
   - If `smiles is not None`: existing path -- call `smiles_to_xyz(smiles)` to generate initial geometry
   - If `smiles is None` and `geometry_file is not None`: call `load_geometry_file(geometry_file)` to get `(mol, xyz_block)`, then continue with DFT optimization steps (generate opt input, run NWChem, extract geometry) -- same as existing flow but with geometry from file instead of SMILES

**scratch_dir_override handling:**
At the start of the function, replace the existing `scratch_dir = job_dir / "scratch"` line:
```python
