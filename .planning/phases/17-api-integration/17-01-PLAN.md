...
must_haves:
  ...
  key_links:
    ...
    - from: "src/qm_nmr_calc/tasks.py"
      ...
      via: "run_ensemble_dft_and_nmr call"
...
Output: Working `run_ensemble_nmr_task` that generates conformers, runs DFT+NMR, computes Boltzmann averages, and saves results. API endpoints dispatch to the correct task based on conformer_mode.
</objective>
...
<task type="auto">
  <name>Task 1: Create run_ensemble_nmr_task and update storage</name>
  <files>
    src/qm_nmr_calc/tasks.py
    src/qm_nmr_calc/storage.py
  </files>
...
2. **Create run_ensemble_nmr_task in tasks.py:**
...
def run_ensemble_nmr_task(job_id: str) -> dict:
    ...
    # ... generate spectrum plots and annotated structure (same as run_nmr_task)
...
3. **Add imports** at top of tasks.py:
...
4. **Copy visualization generation logic** from run_nmr_task to run_ensemble_nmr_task (spectrum plots, annotated structure). Use avg_nmr shifts.
...
```
  ...
  <done>run_ensemble_nmr_task function exists and imports correctly. create_job_directory accepts conformer_mode, conformer_method, max_conformers parameters.</done>
</task>
...
class TestEnsembleTaskDispatch:
    ...
    def test_ensemble_task_filters_nmr_complete_conformers(
        self, mock_get_job_dir, mock_complete, mock_start, mock_update,
...
```
  ...
  <done>Tests pass confirming ensemble task filters to nmr_complete conformers and dispatches correctly.</done>
</task>
...
4. Tests pass: `pytest tests/test_tasks.py -v -k "ensemble"`
5. Existing tests still pass: `pytest tests/ -v --ignore=tests/test_nwchem_integration.py`
</verification>
